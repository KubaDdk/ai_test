#!/usr/bin/env python3
"""
Script to run the HTML analyzer and generate user flows and Cypress tests.
"""
...
from src.crawler.html_analyzer import HtmlAnalyzer

def main():
    """Main entry point for the analyzer script."""
    
    # Set up argument parser
    parser = argparse.ArgumentParser(description="Run the HTML analyzer to extract elements and generate tests")
    ...
    parser.add_argument("--cypress", action="store_true", help="Generate Cypress test file")
    ...
    try:
        ...
        logger.info("Initializing HTML analyzer...")
        analyzer = HtmlAnalyzer(config.config_data)
        ...
        base_filename = args.output_filename or f"analysis_{timestamp}"
        ...
        html_file = None
        if "html_content" in analysis_results:
        ...
        # Save Cypress test if requested
        if args.cypress and "cypress_test" in analysis_results:
